...
class NetworkSimulator:
        ...
        for endpoint in self.endpoints:
            ...
            for _ in range(measurements):
                ...
                if random.random() < 0.05:  # 5% chance of spike
                    ...
                    logger.warning(f"High latency detected for {endpoint}: {latency:.1f}ms")
    ...
    def simulate_throughput_metrics(self) -> Dict[str, float]:
        ...
        metrics['available_bandwidth_mbps'] = baseline_bandwidth_mbps * (100 - current_utilization) / 100
    ...
    def simulate_connectivity_issues(self) -> Dict[str, float]:
        ...
        metrics['total_connection_errors'] = timeout_events + dns_failures + connection_refused
    ...
    def simulate_load_balancer_metrics(self) -> Dict[str, float]:
        ...
        if requests_per_backend:
            ...
            metrics['avg_requests_per_backend'] = sum(requests_per_backend) / len(requests_per_backend)
            metrics['load_balance_variance'] = max(requests_per_backend) - min(requests_per_backend)
        else:
    ...
    def simulate_cdn_metrics(self) -> Dict[str, float]:
        ...
        metrics['bandwidth_savings_percent'] = (bandwidth_saved_gb / origin_bandwidth_gb) * 100
    ...
    def simulate_operations(self) -> Dict[str, float]:
        ...
        packet_loss_score = max(0, 100 - (all_metrics.get('packet_loss_rate', 0) * 1000))
        connectivity_score = max(0, 100 - (all_metrics.get('total_connection_errors', 0) * 20))
